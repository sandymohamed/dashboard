...
const Modal = forwardRef(({ lesson_id }: TModalProps, ref) => {
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<TReview>({
        ...
        <form action="post" onSubmit={handleSubmit(onSubmit)}>
          {records.map((record) => (
            <article key={record.id} className={styles.questionBox}>
                  ...
                  record.choice.map((choice) => (
                    <Controller
                      ...
                      render={({ field: { onChange, value } }) => (
                        <>
                          {Array.isArray(value) && <label key={choice.id}>
                            <input
                              type="checkbox"
                              value={choice.id}
              ...
              {errors[record.id] && <p className="error">{errors[record.id]?.message}</p>}
            </article>
          ))}
